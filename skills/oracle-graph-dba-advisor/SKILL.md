...
Do not run recursive globs over all pack SQL files. If pack inventory is needed,
list immediate pack directories, then open only the selected pack README and the
specific numbered templates justified by evidence.
...
approved dev/test, and controlled invisible-index validation plus promotion and
rollback for production/pre-prod.
...
when validating a newly executed statement. For index validation, include
executable SQL to compare baseline vs after cursor metrics and plan operations,
either after immediate validation SQL or after the application reruns the
statement. Do not say "re-run the SQL_ID" or "use this value as :ANCHOR_ID";
