...
commands before the final summary only in `extended` mode or when the user asks
for exact commands. In `quick-win` mode, provide the shortest safe validation
approach and offer the exact SQL runbook in the extended report. Do not stop at
