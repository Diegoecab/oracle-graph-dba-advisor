...
- `04-degree-outliers.sql`: quantifies high-degree device outliers.
- `05-path-fanout-evidence.sql`: estimates path expansion from device to users to bank accounts.
...
- `__EDGE_TABLE__`: device edge table, normally `E_USES_DEVICE`.
- `__SECOND_EDGE_TABLE__`: secondary edge table, normally `E_WITHDRAWAL_BANK_ACCOUNT`.
- `__ANCHOR_DEVICE_ID__`: high-degree device, normally `D00000001`.
