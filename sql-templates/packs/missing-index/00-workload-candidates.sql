...
candidate_sql AS (
  ...
  WHERE NVL(s.executions, 0) > 0
    ...
    AND UPPER(s.sql_text) NOT LIKE '%DBA_IND_COLUMNS%'
...
plan_graph_access AS (
  SELECT
    ...
    SUM(CASE WHEN p.operation = 'TABLE ACCESS' AND p.options = 'FULL' THEN 1 ELSE 0 END) AS graph_full_scan_steps,
    SUM(CASE WHEN p.operation LIKE '%INDEX%' THEN 1 ELSE 0 END) AS graph_index_steps,
...
scoped_sql AS (
  SELECT
    ...
    NVL(pga.graph_full_scan_steps, 0) AS graph_full_scan_steps,
    NVL(pga.graph_index_steps, 0) AS graph_index_steps,
...
SELECT
  ...
  SUM(graph_full_scan_steps) AS graph_full_scan_steps,
  SUM(graph_index_steps) AS graph_index_steps,
...
FROM scoped_sql
GROUP BY sql_id
HAVING SUM(graph_full_scan_steps) > 0
ORDER BY total_buffer_gets DESC, total_elapsed_sec DESC, graph_full_scan_steps DESC
