...
BEGIN
  EXECUTE IMMEDIATE 'CREATE USER &&downer_user IDENTIFIED BY "' ||
    REPLACE(q'[&&downer_password]', '"', '""') ||
    '" DEFAULT TABLESPACE DATA TEMPORARY TABLESPACE TEMP QUOTA 10G ON DATA';
EXCEPTION
  WHEN OTHERS THEN
...
BEGIN
  EXECUTE IMMEDIATE 'ALTER USER &&downer_user IDENTIFIED BY "' ||
    REPLACE(q'[&&downer_password]', '"', '""') ||
    '" ACCOUNT UNLOCK';
EXCEPTION
  WHEN OTHERS THEN
...
BEGIN
  EXECUTE IMMEDIATE 'CREATE USER &&diag_user IDENTIFIED BY "' ||
    REPLACE(q'[&&diag_password]', '"', '""') ||
    '" DEFAULT TABLESPACE DATA TEMPORARY TABLESPACE TEMP QUOTA 200M ON DATA';
EXCEPTION
  WHEN OTHERS THEN
    IF SQLCODE != -1920 THEN
      RAISE;
...
BEGIN
  EXECUTE IMMEDIATE 'ALTER USER &&diag_user IDENTIFIED BY "' ||
    REPLACE(q'[&&diag_password]', '"', '""') ||
    '" ACCOUNT UNLOCK';
EXCEPTION
  WHEN OTHERS THEN
...
GRANT GRAPH_DEVELOPER TO &&downer_user;
ALTER USER &&downer_user DEFAULT ROLE ALL;
ALTER USER &&downer_user GRANT CONNECT THROUGH GRAPH$PROXY_USER;
