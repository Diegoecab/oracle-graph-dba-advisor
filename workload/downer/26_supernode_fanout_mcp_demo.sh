...
: "${SQL_TAG:=DOWNER_SN_Q01}"
: "${GRAPH_OWNER:=DOWNER_DEMO}"
: "${EDGE_TABLE:=E_USES_IP}"
: "${SECOND_EDGE_TABLE:=E_WITHDRAWAL_BANK_ACCOUNT}"
: "${ANCHOR_ID:=IP00000001}"

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REPO_ROOT="$(cd "${SCRIPT_DIR}/../.." && pwd)"
SQL_TEMPLATE_DIR="${REPO_ROOT}/sql-templates/packs/supernode-fanout"
...
render_sql_template() {
  ...
  rendered="${rendered//__SECOND_EDGE_TABLE__/${SECOND_EDGE_TABLE}}"
  rendered="${rendered//__ANCHOR_ID__/${ANCHOR_ID}}"
  if [[ $# -ge 2 ]]; then
...
SQL_DEGREE="$(render_sql_template "${SQL_TEMPLATE_DIR}/04-degree-outliers.sql")"
SQL_PATH_FANOUT="$(render_sql_template "${SQL_TEMPLATE_DIR}/05-path-fanout-evidence.sql")"
...
if [[ -z "$PRIMARY_SQL_ID" || "$PRIMARY_SQL_ID" == "null" ]]; then
  echo "No primary SQL_ID found for tag ${SQL_TAG}. Run workload/downer/19_run_supernode_workload.sql first." >&2
  exit 2
fi
...
section "Degree Outliers"
call_run_sql "$SQL_DEGREE" | jq .

section "Path Fanout Evidence"
call_run_sql "$SQL_PATH_FANOUT" | jq .
